...
if (empty($mort_term)) {
    echo "Missing required value 'mortgage_term'";
}
...
if (!file_exists($r['calc_file'])) {
    if (!file_exists($_SERVER['DOCUMENT_ROOT'] . "/portal/assets/" . $r['calc_file'])) {
        $r['calc_file'] = $_SERVER['DOCUMENT_ROOT'] . "/advisors_a/" . $r['calc_file'];
    } else {
        $r['calc_file'] = $_SERVER['DOCUMENT_ROOT'] . "/portal/assets/" . $r['calc_file'];
    }
}
...
foreach ($data as $key => $value) {
    ...
    if (substr($value, 0, 5) == "Total") {
        break;
    }

    $month++;
    if ($month > $max_months_display) {
        break;
    }
